...
public class GatewayRouteConfig {
    ...
    public RouteLocator customRouteLocator(RouteLocatorBuilder builder) {
        ...
        return builder.routes()
                .route("host_route", r -> r.path("/a/**").filters(f -> f.stripPrefix(1)).uri("http://localhost:2006"))
                .route("host_route", r -> r.path("/b/**").filters(f -> f.stripPrefix(1)).uri("http://localhost:8902"))
                //使用服务名的方式转发
                .route("host_route", r -> r.path("/c/**").filters(f -> f.stripPrefix(1)).uri("lb://app-game-service"))
                .build();
    }
